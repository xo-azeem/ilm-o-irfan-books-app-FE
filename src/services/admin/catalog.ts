...
export async function listAdminAuthors(query = ''): Promise<AdminAuthor[]> {
  let builder = supabase
    ...
    .select(
      'id,slug,name,bio,avatar_path,created_at,updated_at,book_count,published_count',
    )
    .order('name');
...
export async function upsertAdminAuthor(
  input: AdminAuthorInput,
): Promise<string> {
  const payload = {
  ...
  return unwrap(
    await supabase.from('authors').insert(payload).select('id').single(),
  ).id;
}
...
export async function listAdminCategories(): Promise<AdminCategory[]> {
  return unwrap(
    await supabase
      ...
      .select(
        'id,slug,label,icon_key,accent,accent_dark,sort_order,created_at,book_count',
      )
      .order('sort_order')
      .order('label'),
  ) as AdminCategory[];
...
export async function getCollectionBookIds(
  collectionId: string,
): Promise<string[]> {
  const rows = unwrap(
    await supabase
